...
#Y= yields in mol biomass-N per mol resource-N (empirical yield = 0.25)
...
#Anammox (NO2 + NH4 -> N2)
Y_no2_ana= 0.03 #THIS YIELD HAS BEEN CHANGED TO CREATE LOWER NO2* FOR ANAMMOX
...
#Avg (coexist)
coex= (R_dnra+R_FullDenit)/2 #Same as Net Zero N Loss
...
##Biomass
b_DNRA <- read.csv("~/Fadum_etal_2025_DNRAvDenit/python/DNRA_biomass_Fig4_ModAMX.csv", header=F) #import .py output
...
#Denit (NO2 -> N2O) 
b_denit1=read.csv("~/Fadum_etal_2025_DNRAvDenit/python/IncompDenit_biomass_Fig4_ModAMX.csv", header=F)  #import .py output
...
#Denit (N2O -> N2) 
b_denit2=read.csv("~/Fadum_etal_2025_DNRAvDenit/python/CompDenit_biomass_Fig4_ModAMX.csv", header=F) #import .py output
...
#Anammox heatmapAna
b_ana =read.csv("~/Fadum_etal_2025_DNRAvDenit/python/Anammox_biomass_Fig4_ModAMX.csv", header=F)  #import .py output
...
#NO2 remaining
fin_NO2=read.csv("~/Fadum_etal_2025_DNRAvDenit/python/fin_NO2_Fig4_ModAMX.csv", header=F)  #import .py output
...
#OM remaining
fin_OM=read.csv("~/Fadum_etal_2025_DNRAvDenit/python/fin_OM_Fig4_ModAMX.csv", header=F)  #import .py output
...
#NH4 remaining
fin_NH4=read.csv("~/Fadum_etal_2025_DNRAvDenit/python/fin_NH4_Fig4_ModAMX.csv", header=F)  #import .py output
...
OM_supply = seq(0,10, by=.01) # create NO2 supply rate sequence (full length)
NO2_supply = rep(c(25), times=length(OM_supply)) # restrict OM supply rate 
...
#Find coexist range
str(b_DNRA)
check_coex<- cbind(NO2_supply,OM_supply,b_DNRA,b_denit1,b_ana) %>% 
  mutate(Ratio= OM_supply/NO2_supply) %>% 
  filter(DNRA>1e-04,
         Denit_Step1>1e-04,
         Anammox>1e-04)
...
#ggsave("FigS3A_raw.png", plot = biomassplot, width = 42, height = 8, units = "in", dpi= 600)
...
```{r echo = TRUE, error=FALSE, eval=TRUE, include=FALSE, message=FALSE, warning=FALSE}
## Figure S3B: Relative NO2 uptake
...
#ggsave("FigS3B_raw.png", plot = comp_plot, width = 42, height = 8, units = "in", dpi= 600)
...
#ggsave("FigS3C_raw.png", plot = Balance_plot, width = 42, height = 16, units = "in", dpi= 600)
...
## Figure S5: Percent N2 from anammox
...
Percent_plot<- ggplot(Percent) +
  ...
  ylim(0,100)+ #fix weird axis issue
...
#ggsave("FigS6_raw.png", plot = Percent_plot, width = 42, height = 8, units = "in", dpi= 600)
